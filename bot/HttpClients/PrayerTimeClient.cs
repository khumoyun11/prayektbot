...
namespace bot.HttpClients
{
    public class PrayerTimeClient : IPrayerTimeClient
    {
        ...
        public async Task<(bool IsSuccess, PrayerTime prayerTime, Exception exception)> GetPrayerTimeAsync(double lat, double lon)
        {
            var query = $"/times/today.json?latitude={lat}&longitude={lon}";
